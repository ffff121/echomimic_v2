...
pretrained_weights_path = os.getenv('PRETRAINED_WEIGHTS_PATH')

if pretrained_weights_path is None:
    print("please set pretrained_weights_path and export to PRETRAINED_WEIGHTS_PATH. \nFor example: export PRETRAINED_WEIGHTS_PATH=./pretrained_weights")
...
def generate(image_input, 
    ...
    ############# model_init started #############
    ## vae init
    vae = AutoencoderKL.from_pretrained(f"{pretrained_weights_path}/sd-vae-ft-mse").to(device, dtype=dtype)
    if quantization_input:
        quantize_(vae, int8_weight_only())
        print("int8量化")

    ## reference net init
    reference_unet = UNet2DConditionModel.from_pretrained(f"{pretrained_weights_path}/sd-image-variations-diffusers", subfolder="unet", use_safetensors=False).to(dtype=dtype, device=device)
    reference_unet.load_state_dict(torch.load(f"{pretrained_weights_path}/reference_unet.pth", weights_only=True))
    if quantization_input:
        quantize_(reference_unet, int8_weight_only())

    ## denoising net init
    if os.path.exists(f"{pretrained_weights_path}/motion_module_acc.pth"):
        print('using motion module acc')
    else:
        exit("motion module not found")
        ### stage1 + stage2
    denoising_unet = EMOUNet3DConditionModel.from_pretrained_2d(
        f"{pretrained_weights_path}/sd-image-variations-diffusers",
        f"{pretrained_weights_path}/motion_module_acc.pth",
        subfolder="unet",
    ...
    denoising_unet.load_state_dict(torch.load(f"{pretrained_weights_path}/denoising_unet_acc.pth", weights_only=True),strict=False)

    # pose net init
    pose_net = PoseEncoder(320, conditioning_channels=3, block_out_channels=(16, 32, 96, 256)).to(dtype=dtype, device=device)
    pose_net.load_state_dict(torch.load(f"{pretrained_weights_path}/pose_encoder.pth", weights_only=True))

    ### load audio processor params
    audio_processor = load_audio_model(model_path=f"{pretrained_weights_path}/audio_processor/tiny.pt", device=device)
