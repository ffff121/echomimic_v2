...
def generate(image_input, audio_input, pose_input, width, height, length, steps, sample_rate, cfg, fps, context_frames, context_overlap, quantization_input, seed):
    ...
    ## denoising net init
    if os.path.exists(f"{pretrained_weights_path}/motion_module.pth"):
    ...
    denoising_unet = EMOUNet3DConditionModel.from_pretrained_2d(
        ...
        f"{pretrained_weights_path}/motion_module.pth",
        subfolder="unet",
    ...
    denoising_unet.load_state_dict(torch.load(f"{pretrained_weights_path}/denoising_unet.pth", weights_only=True),strict=False)
